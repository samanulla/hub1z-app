...
def seed_demo_cmd() -> None:
    ...
    if not loc:
        ...
        ground = Floor(tenant_id=tid, location_id=loc.id, level=1, name="Ground — Lounge")
        l5 = Floor(tenant_id=tid, location_id=loc.id, level=5, name="Level 5 — Hot Desks")
        l6 = Floor(tenant_id=tid, location_id=loc.id, level=6, name="Level 6 — Dedicated Desks")
        l7 = Floor(tenant_id=tid, location_id=loc.id, level=7, name="Level 7 — Private Offices")
        db.session.add_all([ground, l5, l6, l7]); db.session.flush()
        for i in range(1, 21):
            db.session.add(Seat(tenant_id=tid, location_id=loc.id, floor_id=l5.id,
                                code=f"L5-HD-{i:03d}", seat_type=SeatType.HOT_DESK,
                                hourly_rate=Decimal("150"), daily_rate=Decimal("900"),
                                monthly_rate=Decimal("12000")))
        for i in range(1, 11):
            db.session.add(Seat(tenant_id=tid, location_id=loc.id, floor_id=l6.id,
                                code=f"L6-DD-{i:03d}", seat_type=SeatType.DEDICATED_DESK,
                                monthly_rate=Decimal("22000")))
        for i in range(1, 6):
            db.session.add(Seat(tenant_id=tid, location_id=loc.id, floor_id=l7.id,
                                code=f"L7-PO-{i:03d}", seat_type=SeatType.PRIVATE_OFFICE,
                                capacity=4, monthly_rate=Decimal("85000")))
        db.session.add_all([
            ConferenceRoom(tenant_id=tid, location_id=loc.id, floor_id=ground.id, code="G-BOARD",
                           name="The Boardroom", capacity=12,
                           hourly_rate=Decimal("2400"), credit_cost_per_hour=2),
            ConferenceRoom(tenant_id=tid, location_id=loc.id, floor_id=l5.id, code="L5-CAUV",
                           name="Cauvery", capacity=6,
                           hourly_rate=Decimal("1200"), credit_cost_per_hour=1),
            ConferenceRoom(tenant_id=tid, location_id=loc.id, floor_id=l5.id, code="L5-KRSH",
                           name="Krishna", capacity=4,
                           hourly_rate=Decimal("800"), credit_cost_per_hour=1),
            ConferenceRoom(tenant_id=tid, location_id=loc.id, floor_id=l7.id, code="L7-EXEC",
